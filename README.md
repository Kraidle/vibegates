...
| "An estimated **7.2 % reduction**" in software delivery **stability** and "an estimated 1.5 % reduction" in throughput "for every 25 % increase in AI adoption" (verbatim, 2024 report pp. 39–40; stability = change failure rate + rework rate, pp. 11–12; N ≈ 3,000). DORA's own hypothesis: AI inflates changelist size, and large changes destabilize delivery. *2025 successor edition: the throughput relationship reversed to positive (treat as not robust), while the **negative stability finding replicated** — the mechanism DORA names is "time saved writing is re-spent auditing" ([Pass-2 evidence record](docs/passes/pass-2-backlog.md))* | DORA / Google Cloud, *Accelerate State of DevOps 2024*, pp. 39–40 ([PDF](https://dora.dev/research/2024/dora-report/2024-dora-accelerate-state-of-devops-report.pdf)) [read — authentic v. 2024.3 page-verified 2026-08-13 after a name-collision counterfeit, see [pass-3 adjudications](docs/passes/pass-3-adjudications.md)] [grey — COI runs *against* the negative finding] |
...
5. Beyond the build gates — the idea phase, release, operations, market, and the agent-organization layer: [docs/05-lifecycle.md](docs/05-lifecycle.md) (draft).
6. Run an [entry audit](templates/entry-audit.md) on each existing project; new projects apply the gates from the first commit.
