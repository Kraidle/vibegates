...
Process metrics improve with AI adoption (code quality, complexity, review and approval speed), yet delivery stability and throughput fall ("an estimated 7.2 % reduction" and "an estimated 1.5 % reduction" respectively, "for every 25 % increase in AI adoption" — a relative increase; 89 % uncertainty intervals displayed in Figure 10 but bounds unprinted), and **product performance shows no association** (+0.2 %, n.s.) while organizational (+2.3 %) and team (+1.4 %) performance rise — "high-performing teams use AI, but products don't seem to benefit." DORA's own hypothesis: AI grows changelists, and large changes are slower and more instability-prone; nothing replaces "the basics — small batch sizes and robust testing." The report also warns that faster reviews may reflect **over-reliance on AI**, not better review [DORA 2024] [read pp. 38–43: process gains p. 38, delivery figures pp. 39–40 (Figure 10 + text), org/team/product outcomes pp. 41–43]. This localizes the damage (integration and delivery, not typing) and directly grounds rule R-25 (bounded batch size) and an anti-metric (review speed is never maximized).
